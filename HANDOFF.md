...
0.1.9 실상품 진단에서 `summaryDetected: false`, 빈 `attempts`와 대표 리뷰 12개가
함께 기록됐다. 리뷰 영역이 아직 로드되지 않은 상태에서 대표 리뷰 여부를 먼저
검사해 전체보기 탐색을 건너뛴 것이 원인이었다. 0.1.10은 리뷰 탭과 리뷰 영역을 먼저
연 다음 대표 리뷰 여부를 판단하고 전체보기 후보를 시도한다. 초기 DOM에 리뷰가 없는
상태부터 전체 목록이 열리는 순서를 회귀 테스트로 고정했다.

0.1.10 실상품 진단에서도 `summaryDetected: false`, 빈 `attempts`가 기록됐다. 초기
화면에 대표 리뷰 외의 리뷰 관련 DOM이 잠시 함께 존재해 대표 리뷰 전용 판정이 다시
빗나간 것이다. 0.1.11은 이 판정을 실행 조건으로 사용하지 않는다. 네이버 리뷰 영역을
연 뒤 전체보기 후보가 있으면 반드시 순차 시도하고, 클릭 전후의 URL 또는 리뷰 목록
서명이 실제로 바뀐 경우에만 전체 목록 전환 성공으로 인정한다.
