...
- 공개 작업 생성은 허용된 쇼핑몰 소스만 받고, IP 시간당·전체 일일 요청 제한을 적용한다.
- 대표 리뷰 원문은 보고서 JSON에 저장하지 않고 `reviews` 테이블(7일 만료)에만 두며,
  조회 시 보존 기간이 남은 경우에만 복원한다.
- 원문은 7일, 집계·보고서는 30일 후 삭제하며, Cron Trigger가 요청과 무관하게 만료
  데이터·토큰·방치 작업을 정기적으로 정리한다.
